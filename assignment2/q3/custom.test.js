...
describe('Custom test', function () {
  ...
  describe('insert pair of leaves, deposit to l1 and withdraw from l2', () => {
    it('should work', async () => {
      const { merkleTreeWithHistory, tornadoPool, token, omniBridge } = await loadFixture(fixture)

      // estimate gas for merkle tree insert pair of leaves
      const gas = await merkleTreeWithHistory.estimateGas.insert(toFixedHex(123), toFixedHex(456))
      console.log('insert gas for a pair of leaves', gas - 21000)

      // create new account representing alice
      ...
      await transaction({
        ...
        recipient: recipient,
        isL1Withdrawal: false, // withdraws on l2
      })

      // check receipient, omnibridge and tornado pool balances are correct
