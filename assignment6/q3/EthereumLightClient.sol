...
// keeps track of state on Ethereum via block headers, allows TokenLockerOnHarmony to verify receipt proof is valid given a block hash and receipt hash
contract EthereumLightClient is Ethash, Initializable, PausableUpgradeable {
    ...
    // number of block confirmations before we consider it finalized
    uint256 private constant DEFAULT_FINALITY_CONFIRMS = 13;

    uint256 public finalityConfirms;

    // initialize Ethereum light client
    function initialize(bytes memory _rlpHeader) external initializer {
        ...
        // Parse rlp-encoded block header into BlockHeader structure
    ...
    // For Eth2Hmy block relayer to add block headers into ELC
    function addBlockHeader(
        ...
        // add it to tracked blocks mapping
    ...
    // Used in TokenLockerOnHarmony to verify receipt exists in provided block in our blocks mapping
    ...
    // Used to initialize light client by storing the genesis block and initializing all our block record data structures with it
