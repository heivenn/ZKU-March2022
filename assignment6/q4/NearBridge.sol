...
contract NearBridge is INearBridge, AdminControlled {
    ...
    // tracks users bond deposit amounts
    mapping(address => uint256) public override balanceOf;

    // initialize NEAR bridge
    constructor(
        Ed25519 ed,
    ...
    // deposits bond amount for a user submitting a NEAR header if no bond deposit exists already
    function deposit() public payable override pausable(PAUSED_DEPOSIT) {
        require(msg.value == lockEthAmount && balanceOf[msg.sender] == 0);
        // set balance of user to ETH bond amount if no balance previously
        balanceOf[msg.sender] = msg.value;
    }

    // allow bond amount to be withdrawn once user's submitted block header has passed challenge period
    ...
    // NEAR uses fraud proof to verify that signature of submitted block header is valid, anyone can challenge it in the 4 hour challenge period
    function challenge(address payable receiver, uint signatureIndex) external override pausable(PAUSED_CHALLENGE) {
        ...
        // penalizes user who submitted the NEAR header by the bond amount because the signature failed the challenge
        balanceOf[lastSubmitter] = balanceOf[lastSubmitter] - lockEthAmount;
        // reset last valid block
        lastValidAt = 0;
        // challenger receives half of the bond amount, other half gets burned
        receiver.call{value: lockEthAmount / 2}("");
    }

    // used to verify whether signature is valid or invalid by challenge
    function checkBlockProducerSignatureInHead(uint signatureIndex) public view override returns (bool) {
        ...
        unchecked {
            ...
            // use Ed25519 contract to check if the signature is invalid
    ...
    function bridgeState() public view returns (BridgeState memory res) {
        if (block.timestamp < lastValidAt) {
            // there exists an untrustedNextEpoch still within a challenge period
    ...
    // user can submit next block to update light client if bond deposit checks pass
    function addLightClientBlock(bytes memory data) public override pausable(PAUSED_ADD_BLOCK) {
        require(initialized, "Contract is not initialized");
        // user needs to have enough balance in the contract for the bond
        ...
        unchecked {
            ...
            // gather new untrusted signature set from submitted block
    ...
    // used to set validators of a given epoch while checking that the number of validators does not exceed the limit
    function setBlockProducers(NearDecoder.BlockProducer[] memory src, Epoch storage epoch) internal {
        ...
        unchecked {
            ...
            // stake cannot exceed 2/3 validators threshhold
            epoch.stakeThreshold = (totalStake * 2) / 3;
        }
    }

    // get all block hashes at a given block height
    ...
    // get all block merkle roots at a given block height
